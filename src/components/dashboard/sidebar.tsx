...
export const Sidebar = memo(function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`
        fixed inset-y-0 left-0 z-50 w-64 md:w-72 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transform transition-transform duration-300 ease-out flex flex-col
      ...
      `}>
        ...
        <div className="flex items-center justify-between h-16 px-6 border-b border-gray-100 dark:border-gray-700">
          <Link href="/" className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gray-900 dark:bg-white rounded-xl flex items-center justify-center">
              <span className="text-white dark:text-gray-900 font-serif text-lg">⚖</span>
            </div>
            <span className="text-xl font-serif font-bold text-gray-900 dark:text-white">LAW.AI</span>
          </Link>
          <button
            onClick={onClose}
            ...
            {navigationItems.map((item) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon className={`h-5 w-5 ${item.isActive ? 'text-white dark:text-gray-900' : 'text-gray-400 dark:text-gray-500 group-hover:text-gray-600 dark:group-hover:text-gray-400'}`} />
                  <span>{item.name}</span>
                </div>
                {item.isActive && <ChevronRight className="h-4 w-4 text-white/60 dark:text-gray-900/60" />}
              </Link>
            ))}
          </div>
        </nav>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-700">
          <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-700 rounded-xl mb-2">
